...
numberWords' :: Num b => String -> [(b, String)]
numberWords' xs = numberWordsHelper 1 (words xs)
  where
    numberWordsHelper _ [] = []
    numberWordsHelper n (x:xs) = (n, x) : numberWordsHelper (n + 1) xs
...
intersperse' :: a -> [a] -> [a]
intersperse' sep xs = intersperseHelper xs
  where
    intersperseHelper [] = []
    intersperseHelper [x] = [x]
    intersperseHelper (x:xs) = x : sep : intersperseHelper xs
...
showList' xs = "[" ++ showListHelper xs ++ "]"
  where
    showListHelper [] = ""
    showListHelper [x] = show x
    showListHelper (x:xs) = show x ++ "," ++ showListHelper xs
